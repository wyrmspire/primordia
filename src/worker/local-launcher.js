...
const SUBSCRIPTION =
  process.env.LOCAL_LAUNCHER_SUB || "primordia-local-launcher-sub";
...
// IMPORTANT: inside Docker we want the service to hit the GCS emulator by host "gcs"
...
function startService(name) {
  ...
  // A POSIX-safe launch script; pulls files from GCS emulator then runs the service
  const launchScript = `
set -euo pipefail
...
`;
  ...
  // Run detached; node will keep it running
  run("docker", [
    "run",
    "-d",
    "--rm",
    ...
    "-e",
    `STORAGE_EMULATOR_HOST=${STORAGE_EMULATOR}`,
    "-e",
  ...
  // Discover mapped host port for 8080/tcp
  const portLine = run("docker", ["port", containerName, "8080/tcp"]);
  const hostPort = portLine.split(":").pop().trim();
  console.log(
    `[local-launcher] STARTED ${name} on http://localhost:${hostPort} (container :8080)`
  );
}
...
async function main() {
  ...
  const hasSub =
    Array.isArray(subs) &&
    subs.some((s) => s.name.endsWith(`/subscriptions/${SUBSCRIPTION}`));
  if (!hasSub) {
  ...
  console.log(
    `[local-launcher] listening on ${TOPIC} (${SUBSCRIPTION}) via ${PUBSUB_EMULATOR}...`
  );
  ...
  sub.on("error", (e) => {
    console.error("[local-launcher] subscription error:", e.message);
  });
}

main().catch((e) => {
  console.error("[local-launcher] fatal:", e);
  process.exit(1);
});
