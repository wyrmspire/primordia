...
export async function proxyHandler(req, res) {
  try {
    ...
    let fetchBody = body;
    if (fetchBody && typeof fetchBody === "object" && (/json/i.test(cleanHeaders["content-type"] || ""))) { fetchBody = JSON.stringify(fetchBody); }
    ...
    const hdrs = {};
    ["content-type","cache-control","date","etag","server","content-length","location"].forEach(h => { const v = resp.headers.get(h); if (v) hdrs[h] = v; });
